import { getMessageTrayContainer, resolveNotificationWidgets, } from "../shell/notification-widgets.js";
...
export class ThemesManager {
    ...
    enableThemes() {
        ...
        this.themeSignalId = messageTrayContainer?.connect("child-added", () => {
            ...
            if (!widgets)
                return;
            const theme = this.settingsManager.getThemeFor(widgets.sourceName, widgets.titleText, widgets.bodyText);
            if (!theme)
                return;
            widgets.source?.set_style(this.makeStyle(theme.appNameColor, theme.appNameFontSize ?? DEFAULT_THEME.appNameFontSize));
            widgets.time?.set_style(this.makeStyle(theme.timeColor, theme.timeFontSize ?? DEFAULT_THEME.timeFontSize));
            widgets.title?.set_style(this.makeStyle(theme.titleColor, theme.titleFontSize ?? DEFAULT_THEME.titleFontSize));
            widgets.body?.set_style(this.makeStyle(theme.bodyColor, theme.bodyFontSize ?? DEFAULT_THEME.bodyFontSize));
            const existingStyle = widgets.container?.get_style() ?? "";
            widgets.container?.set_style(`${existingStyle} ${this.makeColorStyle(theme.backgroundColor, "background")}`);
        });
    }
}
