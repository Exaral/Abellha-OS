...
const POSITION_VALUES = ["fill", "left", "center", "right"];
const VERTICAL_POSITION_VALUES = [
    "fill",
    "top",
    "center",
    "bottom",
...
export default class NotificationConfiguratorPreferences extends ExtensionPreferences {
    ...
    loadData() {
        this.globalConfig = SettingsManager.parseGlobalConfiguration(this.settings.get_string("global") ?? "{}");
        this.patterns = SettingsManager.parsePatternConfigurations(this.settings.get_string("patterns") ?? "[]");
    }
    saveGlobal() {
    ...
    buildGlobalPage(page) {
        ...
        this.addConfigurationGroups(page, this.globalConfig, () => this.saveGlobal(), null);
        this.addTestSection(page);
    }
    buildPatternsPage(window, page) {
        const patternsGroup = new Adw.PreferencesGroup({
            title: _("Notification Patterns"),
            description: _("Per-pattern overrides that apply to matching notifications"),
        });
    ...
    openPatternDetail(window, index) {
        ...
        const appNameRow = this.createRegexEntryRow(_("App Name"), pattern.matcher.appName, (value) => {
            pattern.matcher.appName = value;
            this.savePatterns();
        });
        matcherGroup.add(appNameRow);
        const titleRow = this.createRegexEntryRow(_("Title"), pattern.matcher.title, (value) => {
            pattern.matcher.title = value;
            this.savePatterns();
        });
        matcherGroup.add(titleRow);
        const bodyRow = this.createRegexEntryRow(_("Body"), pattern.matcher.body, (value) => {
        ...
        this.addConfigurationGroups(detailPage, pattern, () => this.savePatterns(), pattern.overrides);
    ...
    addConfigurationGroups(page, config, onSave, overrides) {
        ...
        if (overrides) {
            this.addOverrideRow(notificationCenterGroup, overrides, "notificationCenter", onSave, updateNotificationCenterVisibility);
        }
        ...
        rateLimitActionRow.set_selected(config.rateLimiting.action === "hide" ? 1 : 0);
        ...
        if (overrides) {
            this.addOverrideRow(rateLimitGroup, overrides, "rateLimiting", onSave, updateRateLimitVisibility);
        }
        ...
        if (overrides) {
            this.addOverrideRow(timeoutGroup, overrides, "timeout", onSave, updateTimeoutVisibility);
        }
        ...
        if (overrides) {
            this.addOverrideRow(urgencyGroup, overrides, "urgency", onSave, updateUrgencyVisibility);
        }
        ...
        const activateInsteadRow = new Adw.SwitchRow({
            ...
            subtitle: _("Switch directly to the window instead of showing a notification"),
        });
        ...
        if (overrides) {
            this.addOverrideRow(windowAttentionGroup, overrides, "windowAttention", onSave, updateWindowAttentionVisibility);
        }
        ...
        const horizontalIndex = POSITION_VALUES.indexOf(config.display.notificationPosition);
        ...
        const verticalIndex = VERTICAL_POSITION_VALUES.indexOf(config.display.verticalPosition);
        ...
        if (overrides) {
            this.addOverrideRow(displayGroup, overrides, "display", onSave, updateDisplayVisibility);
        }
        ...
        const shouldHideHeaderThemeRows = () => !overrides || overrides.colors || overrides.margins
        ...
        const themeRows = this.addThemeEditor(appearanceGroup, config.colors.theme, onSave);
        ...
        const updateThemeRowsVisibility = () => {
            ...
            for (const themeRow of themeRows) {
                themeRow.row.set_visible(active &&
        ...
        if (overrides) {
            this.addOverrideRow(appearanceGroup, overrides, ["colors", "margins"], onSave, updateAppearanceVisibility);
        }
    ...
    addOverrideRow(group, overrides, key, onSave, updateVisibility) {
    ...
    addThemeEditor(_group, theme, onSave) {
        const rows = [];
    ...
    createRegexEntryRow(title, initialValue, onChanged) {
