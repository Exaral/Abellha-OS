...
import { getBannerBin, getMessageTrayContainer, hideBannerAppTitleRow, resolveNotificationWidgets, } from "./notification-widgets.js";
...
export class NotificationsManager {
    ...
    isVerticalAlignTop() {
        return (getBannerBin()?.get_y_align() === Clutter.ActorAlign.START ||
    ...
    patchAnimations() {
        const proto = MessageTray.MessageTray
            .prototype;
        const self = this;
        this.injectionManager.overrideMethod(proto, "_showNotification", (original) => function () {
            original.call(this);
            if (!self.isVerticalAlignTop()) {
                this._bannerBin.y = 0;
            }
        });
        this.injectionManager.overrideMethod(proto, "_updateShowingNotification", (original) => function () {
            if (self.isVerticalAlignTop()) {
                original.call(this);
                return;
            }
        ...
        this.injectionManager.overrideMethod(proto, "_hideNotification", (original) => function (animate) {
            if (self.isVerticalAlignTop()) {
    ...
    setupPositioning(settingsManager) {
        ...
        Main.messageTray.bannerAlignment =
            NotificationsManager.HORIZONTAL_ALIGNMENT_MAP[settingsManager.notificationPosition];
        bannerBin?.set_y_align(NotificationsManager.VERTICAL_ALIGNMENT_MAP[settingsManager.verticalPosition]);
        ...
        settingsManager.events.on("verticalPositionChanged", (verticalPosition) => {
            bannerBin?.set_y_align(NotificationsManager.VERTICAL_ALIGNMENT_MAP[verticalPosition]);
        });
        ...
        this.positionSignalId = messageTrayContainer?.connect("child-added", () => {
            ...
            const position = settingsManager.getPositionFor(sourceName, titleText, bodyText);
            Main.messageTray.bannerAlignment =
                NotificationsManager.HORIZONTAL_ALIGNMENT_MAP[position];
            const verticalPosition = settingsManager.getVerticalPositionFor(sourceName, titleText, bodyText);
            bannerBin?.set_y_align(NotificationsManager.VERTICAL_ALIGNMENT_MAP[verticalPosition]);
            const shouldHideAppTitleRow = settingsManager.shouldHideAppTitleRowFor(sourceName, titleText, bodyText);
            const margins = settingsManager.getMarginsFor(sourceName, titleText, bodyText);
            if (margins) {
                widgets.container.set_style(`margin-top: ${margins.top}px; margin-bottom: ${margins.bottom}px; margin-left: ${margins.left}px; margin-right: ${margins.right}px;`);
            }
