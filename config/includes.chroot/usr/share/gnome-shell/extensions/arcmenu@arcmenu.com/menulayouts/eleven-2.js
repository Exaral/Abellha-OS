...
export class Layout extends BaseMenuLayout {
    ...
    constructor(menuButton) {
        ...
        // Styled as a pill-shaped button row
        const navButtonsStyle = 'padding: 8px 20px; margin: 4px 12px; border-radius: 999px; background-color: rgba(255, 255, 255, 0.03);';
        ...
        for (let i = 0; i < extraButtons.length; i++) {
            ...
            if (id === Constants.ShortcutCommands.SEPARATOR) {
                ...
                if (button.shouldShow) {
                    // Force extra command buttons into rounded pill shapes
                    button.style = (button.style || '') + ' border-radius: 999px; padding: 6px 12px;';
    ...
    updateStyle() {
        ...
        const style = `margin: 0px; spacing: 6px; background-color: rgba(10, 10, 15, 0.1); padding: 12px 25px;
        ...
        // Apply pill styling dynamically to any button children in actionsBox
        for (let i = 0; i < this.actionsBox.get_n_children(); i++) {
            const child = this.actionsBox.get_child_at_index(i);
            if (child && child.style_class?.includes('button')) {
                child.style = (child.style || '') + ' border-radius: 999px;';
            }
        }
    }
