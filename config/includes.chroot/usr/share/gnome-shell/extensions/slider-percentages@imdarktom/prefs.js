...
export default class SliderPercentagesPreferences extends ExtensionPreferences {
    static createLabelPrefsRows({ settings, group, optionKeyPrefix }) {
        const prefix = (key) => `${optionKeyPrefix}${key}`;
        ...
        const customFontListenerId = customFontFamilyRow.connect('destroy', () => {
        ...
        settings.bind(prefix('label-custom-font-family'), customFontFamilyRow, 'text', Gio.SettingsBindFlags.DEFAULT);
    ...
    static createSwitchRow({ title, subtitle, group, settings, key }) {
